...
var dtd=$.Deferred(),dtdTs=$.Deferred();
...
function urlParse(search){
    ...
    if(typeof urlObj.courseNumber=='undefined'||typeof urlObj.faultNumber=='undefined'||typeof urlObj.domainUrl=='undefined'||typeof urlObj.userEmail=='undefined'||typeof urlObj.domainAccount=='undefined'||typeof urlObj.userCourseClassId=='undefined'){
        //window.location.href="http://content.xiaochejiang.com/warn.html";
    }
    return urlObj;
}
...
Adapter.prototype.getStudyTs = function (dtd) {
    ...
    judgeCookie(userEmail);
   var data1={"email":this.userEmail,"courseNumber":this.courseNumber,"faultNumber":"","domainUrl":domainUrl,"domainAccount":this.domainAccount,"userCourseClassId":this.userCourseClassId};
 ...
 */
  function judgeCookie(){
    ...
    //var reg=new RegExp(userEmail);
      if(!(/XCJSESSID/.test(cookie))){
        handleError('20001');
       }
  };
 ...
 * @param handle 动作数组对象
 */
Adapter.prototype.startStudy=function(progress,paper,handle,initCallback){
    var _this=this;
    url='http://'+location.host+startStudyUrl;
    var obj={};
    obj.adapter=JSON.stringify(_this);
    $.ajax({
        type:"POST",
        url:url,
        dataType:"json",
        ...
        success:function(data){
            ...
            if(data.errCode=="0") {
               ...
               // console.log(JSON.stringify(order));
                ...
                _this.analyzeCharacter(data.adapter.characterA, data.adapter.characterB, data.adapter.characterC,progress, paper, handle);
                progress.rate = _this.progress;
                paper.rate = _this.score;
                initCallback && initCallback(data);
            }else{
                handleError(data.errCode);
            }
            if(data==""){
 ...
 * @param handle 动作数组对象
 */
Adapter.prototype.analyzeCharacter = function (characterA, characterB,characterC, progress, paper, handle) {
    ...
    // console.log("开始解析characterC: ", characterC);
    characterC=JSON.parse(characterC);
    if(characterC && characterC.customData){
        for(keyname in characterC.customData){
            handle.setCustomData(keyname, characterC.customData[keyname]);
        }
    };

    if(characterC && characterC.answers){
        // console.info(characterC.answers)
        for (var item in characterC.answers) {
            // console.info(characterC.answers);
            myQuestion.setAnswers(item, characterC.answers[item]);
        };
    };
    // console.info("json.parse之后的数据: ", characterC)
    // if(characterC!=[]&&characterC){
    //     for(var y=0;y<characterC.length;y++){
...
Adapter.prototype.composeCharacter = function (progress, paper, handle) {
    ...
    this.characterC = {};
    var obj = {};
    handle.reHandle.eachMap(function(key,value){
        obj[key] = value;
    });
    this.characterC.customData = obj;
    this.characterC.answers = myQuestion.getAnsers();
    this.progress = progress.rate;
    //this.score = paper.rate;
    this.score = myQuestion.cacluteScore().score;
    // console.info("提交远程的数据: ", this);
