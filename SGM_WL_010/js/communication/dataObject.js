...
ProgressArray.prototype.sumRate = function() {
    ...
    this.rate = allRate ? (sum / allRate * 100).toFixed(2) : 0;
};
...
HandleArray.prototype.setHandle = function(handleID, progress, paper, time, flag, sub) {
    flag = flag || 0;
    sub = sub || 0;
    var preIsAction, paperFlag = true,
        progressFlag = true;
    if (handleID == "") return;
        ...
        if (progress.progressMap) {
            if (progress.progressMap.find(this.actionMap.get(handleID).PID)) {
                if (!preIsAction) {
                    ...
                    if (progressFlag) {
                        progress.progressMap.get(this.actionMap.get(handleID).PID).isPass = true;
...
HandleArray.prototype.setCustomData = function(key, value){
    this.reHandle.map[key] = value;
};

HandleArray.prototype.setAnswers = function(questionId, answer){
    if(!this.reHandle.answer){
        this.reHandle.answer = [];
    }
    this.reHandle.answer.push({
