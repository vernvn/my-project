...
export class DashboardService {
    dackspeed: number = 0;
    fanisOK : boolean = false;   //判断风扇是否可以转动
    stalls = 0; // 档位在lock档
    /**
     * 储存lock档位需要执行哪些外部方法
     * 
    ...
    */
    ...
    Mout = {
        ...
        Out1Mot_x_Power_x_x: '0',  //功率表指针参数
        Out1Mot_x_VehSpeed_x_x: 0  //车速
    ...
    //风扇进行动画
    fananimateArr = [];
    fananimateIndex : number;
    setFanAnimate(fun){
        this.fananimateArr[this.fananimateIndex] = fun;
        this.fananimateIndex+=1;
    };
    fananimate(){
        for (let i in this.fananimateArr) {
            this.fananimateArr[i]();
        }
    };
    //停止风扇进行动画
    stopfananimateArr = [];
    stopfananimateIndex : number;
    setStopFanAnimate(fun){
        this.stopfananimateArr[this.stopfananimateIndex] = fun;
        this.stopfananimateIndex+=1;
    };
    stopfananimate(){
        for (let i in this.stopfananimateArr) {
            this.stopfananimateArr[i]();
        }
    }
}